...
def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        # TODO multi-object mode tracking
        if isinstance(seq.ground_truth_rect, (dict, OrderedDict)):
            anno_bb = torch.tensor(seq.ground_truth_rect[seq.object_ids[0]], dtype=torch.float64)
        else:
        ...
        for trk_id, trk in enumerate(trackers):
            ...
            # TODO Fix results path based on multi-object tracking sequences ...
            if seq.object_ids is None:
                results_path = '{}.txt'.format(base_results_path)
            else:
                results_path = '{}_{}.txt'.format(base_results_path, seq.object_ids[0])  # TODO multiple object ids
