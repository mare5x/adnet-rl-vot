...
def merge_multiple_runs(eval_data):
    ...
    for i in range(len(trackers)):
        ...
        # My workaround for distinguishing experiments without new params file
        # Also group by display_name
        match = [t['name'] == base_tracker['name'] and t['param'] == base_tracker['param'] and t['disp_name'] == base_tracker['disp_name'] for t in trackers]
...
def print_per_sequence_results(trackers, dataset, report_name, merge_results=False,
    ...
    return [s for s, v in zip(dataset, valid_sequence.tolist()) if v]
