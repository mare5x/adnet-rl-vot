...
class Display:
    ...
    def key_callback_fn(self, event):
        if event.key == ' ':
            ...
            self.active = False
        elif event.key == 'r':  # Reset to start
            self.frame_number = 0
            self.pause_mode = True
            self.step_size = 0
            self.step_direction = 'forward'
...
def playback_results(trackers, sequence):
    ...
    for trk_id, trk in enumerate(trackers):
        ...
        # Fix results path based on multi-object tracking sequences ...
        if sequence.object_ids is None:
            results_path = '{}.txt'.format(base_results_path)
        else:
            results_path = '{}_{}.txt'.format(base_results_path, sequence.object_ids[0])
...
if __name__ == "__main__":
    # NOTE: Press 'q' or 'Esc' to quit the visualization! (See Display.key_callback_fn)

    trackers = []
    trackers.extend(trackerlist('atom', 'default', None, 'ATOM'))
    trackers.extend(trackerlist('atom_rl', 'default', None, 'ATOM_RL'))
    trackers.extend(trackerlist('dimp', 'dimp50', None, 'DiMP50'))

    dataset = get_dataset('dv2017_val')

    playback_results(trackers, dataset[3])
