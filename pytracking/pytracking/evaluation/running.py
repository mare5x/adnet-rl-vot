...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, visdom_info=None, force=False):
    ...
    if not force and _results_exist() and not debug:
        print('FPS: {}'.format(-1))
        return
    ...
    if force or not debug:
        _save_tracker_output(seq, tracker, output)


def run_dataset(dataset, trackers, debug=False, threads=0, visdom_info=None, force=False):
    ...
    if mode == 'sequential':
        for seq in dataset:
            for tracker_info in trackers:
                run_sequence(seq, tracker_info, debug=debug, visdom_info=visdom_info, force=force)
    elif mode == 'parallel':
        param_list = [(seq, tracker_info, debug, visdom_info, force) for seq, tracker_info in product(dataset, trackers)]
