...
def make_subsequence(seq, first, last):
    """Make Sequence from seq with frames [first, last]."""
    args = {
        'name': f"{seq.name}_{last - first}_{first}_{last - 1}",
        'frames': seq.frames[first:last],
        'dataset': seq.dataset,
        'ground_truth_seg': seq.ground_truth_seg[first:last] if seq.ground_truth_seg else seq.ground_truth_seg,
        'init_data': None, 
        'object_class': seq.object_class,
        'target_visible': None, 
        'object_ids': seq.object_ids,
        'multiobj_mode': seq.multiobj_mode
    }
    if isinstance(seq.ground_truth_rect, (dict, OrderedDict)):
        args['ground_truth_rect'] = { k: v[first:last] for k, v in seq.ground_truth_rect.items() }
    else:
        args['ground_truth_rect'] = seq.ground_truth_rect[first:last]

    return Sequence(**args)


def flatten_sequence(seq):
    """Split multi object sequences into separate single object sequences, one for each object."""

    def _flat_single(s, obj_id):
        s = copy.copy(s)
        s.name = f"{s.name}_{obj_id}"
        s.ground_truth_rect = s.ground_truth_rect[obj_id]
        s.object_ids = None
        s.multiobj_mode = None
        return s

    seqs = SequenceList()
    if seq.object_ids is not None:
        for obj_id in seq.object_ids:
            seqs.append(_flat_single(seq, obj_id))
    else:
        seqs.append(seq)
    return seqs
