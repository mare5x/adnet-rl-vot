...
from pytracking.evaluation.data import Sequence, SequenceList, make_subsequence, flatten_sequence
...
class RandomSequenceSampler(torch.utils.data.Dataset):
    def __init__(self, datasets, num_frames, samples_per_epoch=-1):
        """
        Sample a random subsequence of a given length from multiple datasets.

        args:
            datasets - List of datasets to be used for training
            num_frames - Number of frames in sampled sequence (-1 for whole sequence)
            samples_per_epoch - Number of training samples per epoch (-1 for the number of sequences)
        """
        # Merge datasets into a SequenceList
        sequences = []
        for dataset in datasets:
            if isinstance(dataset, list):
                sequences.extend(dataset)
            elif isinstance(dataset, Sequence):
                sequences.append(dataset)
            else:
                for seq in dataset.get_sequence_list():
                    # Split multi object sequences into separate sequences, one for each object
                    sequences.extend(flatten_sequence(seq))
        sequences = [seq for seq in sequences if len(seq.frames) >= num_frames]
        self.sequences = SequenceList(sequences)
        self.num_frames = num_frames
        self.samples_per_epoch = samples_per_epoch if samples_per_epoch > 0 else len(self.sequences)
    ...
    def __getitem__(self, index):
        ...
        returns:
            Sequence - Sequence of length num_frames
        """
        seq = random.choice(self.sequences)
        n = len(seq.frames)
        num_frames = n if self.num_frames == -1 else self.num_frames

        first = 0 if num_frames >= n else random.randrange(n - num_frames)
        last = first + num_frames
        
        # Skip sub-sequences that start with an unknown ground truth
        if -1 not in seq.ground_truth_rect[first, :]:
            subseq = make_subsequence(seq, first, last)
        else:
